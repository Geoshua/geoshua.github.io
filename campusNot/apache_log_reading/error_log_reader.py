...
error_list_components = [
    {'client IP': re.search(r'(\[.*?\]) (\[.*?\]) (\[.*?\]) (\[.*?\]) (.*)', error_item).group(4)[8:-6],
...
for item in graph_timestamp_universal.items():
    print(item)
for item in graph_timestamp_ip.items():
    print(item)
for item in graph_timestamp_error.items():
    print(item)
